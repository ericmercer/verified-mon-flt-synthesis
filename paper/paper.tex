...
\begin{abstract}
Safety critical systems such as avionics need to be engineered to be cyber-resilient meaning that systems are able to detect and recover from attacks or safely shutdown.
%
...
In recent years, stakeholders have realized that avionics systems are subject to possible cyber-attacks just like other cyber-physical systems.
%
...
Unfortunately, systems designers are given few development tools to help answer even basic questions about potential vulnerabilities and ways to mitigate vulnerabilities.
%
...
The main concept is that of \emph{security-improving transform} through the insertion of new components aimed at mitigating cyber-vulnerabilities.
%
...
The contributions of this paper relate to the aspects of \brfcs\ concerning evidence that such components meet their intended purpose and are properly implemented.
%
...
%
The verified compiler, with the semantics preserving transforms, ensure that the high-assurance component adheres to its contract specification.
%
...
We report on a case study applying these transformations with \brfcs\ to an Unmanned Aerial Vehicle (UAV) system.
%
That system uses the Air Force Research Laboratory's OpenUxAS services for route planning.
%
...
Our approach currently applies to common vulnerabilities such as buffer overflow, lack of input validation, and supply-chain issues.
%
...
The behavior of the SW system, and the components implementing the system, are modeled with \agr\ contract specifications as shown by the unshaded boxes in the top-left of \figref{fig:workflow}.
%
...
\begin{compactitem}
  ...
  \item further waypoints coincide with location updates.
\end{compactitem}

This behavioral definition of the UAV software system is the starting point on the left-side of \figref{fig:workflow}.
%
...
After \agr\ verification, the workflow in \figref{fig:workflow} proceeds to the thread analysis as the UAV must actively protect itself from malicious actors.
%
...
As \agr\ verifies the system now meets its cyber-requirements, the workflow moves to the right in \figref{fig:workflow} to the code generation box.
%
...
A system implementation is described by its ports, its sub-components, and the connections between ports on different sub-components.
%
...
Data flows through the sub-components of a system in dependency order: the inputs propagate to outputs through the contracts until everything stabilizes.
%
Therefore, sub-component contracts, and thus the top-level model, must be acyclic.
%
...
A \emph{system} is a tuple $(\inputs, \outputs, \assumes, \guarantees,C)$, where $\inputs$ is the input set, $\outputs$ is the output set, $\assumes$ is the set of assumptions, $\guarantees$ is the set of guarantees, and $C$ are sub-components.
%
Assumptions and guarantees only use the past-time operators from PLTL.
%
A sub-component $\component$ is, hierarchically, also a system, and may be designated by its own tuple $(\inputs_\component, \outputs_\component, \assumes_\component, \guarantees_\component, C_c)$.
%
...
\]
%
Given an \emph{environment} $E : \konst{name} \mapsto \konst{stream}$ binding variable names to value streams, the semantics $\sem{-}^E_t$ of terms and formulas defines the meaning of compound syntax in terms of the meaning of sub-expressions.
%
...
%
the alert is signaled if the start does not arrive within one-step.
%
...
How does this definition of code contract correctness integrate with the verification conditions generated by \agr?
%
...
%
That requirement is encoded in an assumption (\lineref{line:mon-assume}) and is the same assumption used for the system input.
%
Although the policy can be written without the assumption, it is much simpler to write with the assumption.

\figref{fig:hardened-certificate} shows the hardened system to pass AGREE verification.
%
...
%
Our case studies have shown that designers, as well as formal methods practitioners, make mistakes writing these contracts, especially if the computation or temporal reasoning in the contract is complex.
...
A tempting approach to exercising code contracts directly is to use synthesis to create the target binary and then use the target runtime with traditional testing.
%
Such an approach adds extra steps, including test harnesses, that are not needed, as it is possible to bridge the formal model in \agr, with its proof system, to traditional test validation.
%
...
Test contracts are a tool to help in detecting vacuity, under-specification, and missed corner cases.
%
...
%
In this paper, we extend this specification oriented approach to the Turing-complete specifications of code contracts in \agr.
%
We give a short review of our previous work before going into detail on the synthesis.

\paragraph{Regular expressions:\/} often the well-formed property of data can be enforced by matching against a regular expression.
%
...
%
Then we can directly create a logic function
%
\[
...
\item Add needed parsers and printers (specified by contiguity types) for input/output.
...
The synthesized code for \figref{fig:filter-spec} is shown in \figref{fig:filter-cakeml}.
%
...
%
It is an obligation on the system that the scheduler follows some sensible total order of component activation and allows each component sufficient time for its computation.
%
...
\paragraph{Partiality of the Correctess Argument:\/} partiality is an important consideration for both filters and monitors as the input data might not parse or the computation of a component could be badly written and fail at runtime.
%
In such cases, the component should recover and yield control without passing the input onwards.
%
For these failure cases, the component is behaving as it should, but we must also guard against situations in which a correctly specified filter or monitor fails in the runtime.
%
...
%
Another motivation was to determine whether or not the technology provides designers with early validation of their models,
%
...
We modeled the system architecture of the UAV in AADL.
%
...
%
We performed a formal analysis with \agr\ to verify that the model satisfies the contracts.
%
For the initial version of our design, the verification passed.
%
...
%
These include four well-formed requirements on data, two requirements for monitoring the behavior of the open-source UxAS component, and an attestation requirement for ensuring the integrity of the Ground Station software.
%
Requirements were imported into the model as goals in the assurance case.
%
...
The well-formed data requirements serve to prevent any malformed messages from causing a buffer overrun or code injection attack.
%
In the UAV design, such messages are most likely to originate from a remote source or the uncontrolled UxAS component.
%
By placing filters on the connections upstream of mission-critical components, such attacks are mitigated.
%
We therefore inserted filter components on the incoming and outgoing UxAS connections.

The code contracts for these filters are similar in that they check that values contained in the messages are within appropriate ranges.
%
...
%
This required adding components for detecting that UxAS has crashed, as well as monitoring the correctness of the flight plans it produces.
%
We therefore added a response monitor (Resp Monitor) to send an alert if UxAS does not emit a response within a set amount of time from receiving a request.
%
And we added a geo-fence monitor (Geofence Monitor) to ensure that generated flight plans are compliant with the specified keep-in and keep-out zones.
%
...
%
\brfcs\ includes the tools to generate the component stubs and infrastructure code necessary to enable component communication and execution according to a specified schedule.
%
These tools were used here to target the verified micro-kernel seL4.
%
...
\paragraph{Infected Ground Station:\/} an application file is modified on the Ground Station to send the UAV on a mission-violating trajectory.
%
...
%
The work in this paper differs in that it is targeting a fully verified path from the code contract all the way to the binaries leveraging the \ckml\ compiler specifically to provide stronger guarantees on the high-assurance components.
...
%
We have discussed tools provided by \brfcs\ to insert, create, and synthesize high-assurance components such as filters and monitors to improve the cyber-resiliency of the system design.
...
%
A code contract is a subset of the \agr\ language with formal semantics for formal reasoning.
%
...
Code contracts are validated with unit tests that are verified by \agr.
%
A unit test is expressed in a test contract in the \agr\ language.
%
...
%
This reported case study was carried out by those that created \brfcs\ with its several tools.
...
we are conducting another case study involving system designers unfamiliar with the \brfcs\ toolchain.
%
This study is with the design of an application using the Collins Common Avionics Architecture System (CAAS)~\cite{caas} on the CH-47F Chinook helicopter program.
%
...
%
The code contract semantics are embedded in HOL4 already.  The key remaining steps are:
%
\begin{compactenum}
  ...
  \item create a step based version of the modified code contract that does not use streams and prove it equivalent to the stream version; and
...
%
We have additionally written HOL4 proofs for every step above for a set of examples.
%
From these proofs, we have started work on automatically generating proofs for any input.
%
The first two steps above are universal to all input code contracts so their proofs need to be generated just once (as opposed to generating them for every component contract).
%
The proof for the third step must be automatically generated for each input contract.
%
