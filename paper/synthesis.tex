...
occurrence it generates a CakeML program. Starting from the semantics
of the code contract described in Section \ref{sec:code-contracts}
a sequence of semantics-preserving steps are taken:
\begin{itemize}
\item $\sem{{\Eqs}\cdot G}$ yields a function on arbitrary depth streams
\item transform to function over depth-1 streams (wellformedness)
