...
It differentiates with the followed-by operator what is required at time 0, the start must coincide with the request, with what is required after time 0, the start must coincide with the request or is a response to a request one step earlier.
...
The \texttt{Once} operator is true if the enclosed expression has ever been true before, or in, the current moment.
