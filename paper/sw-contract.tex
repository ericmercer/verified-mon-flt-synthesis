...
The contract has six guarantees:
\begin{itemize}
\item \linesref{line:sw-guarantee-1}{line:sw-guarantee-3}
coincide events and assert well-formed output.\footnote{\konst{KLS}:I am confused about the ``coincide events''.}
\item \lineref{line:sw-guarantee-4} insists that a start happens with a request or one step after a request. At time zero the start must coincide
...
\item \lineref{line:sw-guarantee-5} forces the alert to sound if the start does not arrive within the one-step bound. Together with \lineref{line:sw-guarantee-4} this allows
for non-alerting and alerting behavior.
\item \lineref{line:sw-guarantee-6} guarantees that, if an alert has ever happened,
