...
From time to time, SW receives an \texttt{AutomationRequest} message,
...
changes.\footnote{\konst{KLS}:What about Alert? When raised, is it
persistent, where is it computed?}  The AI component is third-party
...
the contract for SW assumes that its inputs are well-formed and that
there is never more than one automation request pending at a time.
(\emph{Well-formed} generally refers to a syntactic restriction on
...
for SW ensure that
\begin{compactitem}
\item a start includes a waypoint;\footnote{\konst{KLS}: needs clarification; Start is just an event port.}
...
\end{compactitem}
These initial specifications pass AGREE verification, meaning that the
...
%% program.
 ...
 high-assurance component's guarantees.  The CakeML compiler rovides
